...
- Timeouts — and why a missing one defeats every other mechanism
- Retries with backoff and jitter
- Circuit breakers
- Bulkheads and isolation
- Graceful degradation and fallbacks
- Load shedding and goodput collapse
- Health checks: liveness vs readiness, and the deep-check outage
...
- [`notebooks/04_timeouts_and_graceful_degradation.ipynb`](./notebooks/04_timeouts_and_graceful_degradation.ipynb) — thread-pool exhaustion from missing timeouts (run, not described), proof that a breaker can't trip without one, hard timeouts, fallback to defaults / stale cache, feature-flag kill switches, load shedding, and health checks; ends with the full stack diagram and a when-NOT-to-use column for every pattern.
